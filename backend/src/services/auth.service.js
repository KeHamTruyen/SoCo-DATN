...
class AuthService {
    ...
    async resendVerification(email) {
        ...
        if (!user) {
            ...
            return {
                message:
                    "If that email is registered, a new code has been sent.",
            };
        }
        if (user.isVerified) {
            throw Object.assign(new Error("Account is already verified"), {
                statusCode: 400,
            });
        }
    ...
    async login({ email, password }) {
        ...
        if (user.twoFactorAuth?.isEnabled) {
            ...
            const tempToken = this._signPurposeToken(
                user.id,
                "2fa-pending",
                "10m",
            );
    ...
    async verify2FA(tempToken, otpCode) {
        ...
        if (!tfa || !tfa.isEnabled) {
            throw Object.assign(new Error("2FA is not enabled"), {
                statusCode: 400,
            });
        }
    ...
    async enable2FA(userId) {
        ...
        return {
            message:
                "OTP sent to your email. Submit it to confirm 2FA activation.",
            backupCodes,
        };
    }
    ...
    async confirm2FAEnable(userId, otpCode) {
        ...
        if (!tfa)
            throw Object.assign(new Error("Please initiate 2FA setup first"), {
                statusCode: 400,
            });
        ...
        for (const key of allowed) {
            if (settings[key] !== undefined) {
                if (!validValues[key].includes(settings[key])) {
                    throw Object.assign(new Error(`Invalid value for ${key}`), {
                        statusCode: 400,
                    });
                }
