...
class AuthController {
  ...
  async register(req, res, next) {
    try {
      ...
      const result = await authService.register({ email, username, password, fullName, phone, role });
  ...
  async confirm2FAEnable(req, res, next) {
    try {
      ...
      const result = await authService.confirm2FAEnable(req.user.id, otpCode);
      res.json({ success: true, data: result });
    } catch (error) {
      next(error);
    }
  ...
  async updatePrivacy(req, res, next) {
    try {
      const settings = await authService.updatePrivacySettings(req.user.id, req.body);
