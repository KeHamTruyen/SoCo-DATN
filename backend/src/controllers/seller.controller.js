...
class SellerController {
    ...
    async startApplication(req, res, next) {
        try {
            const application = await sellerService.getOrCreateApplication(
                req.user.id,
            );
    ...
    async withdrawReviewingApplication(req, res, next) {
        try {
            const result = await sellerService.withdrawReviewingApplication(
                req.user.id,
            );
    ...
    async submitStep1(req, res, next) {
        try {
            const result = await sellerService.submitStep1(
                req.user.id,
                req.body,
            );
            res.json({
    ...
    async submitStep2(req, res, next) {
        try {
            const result = await sellerService.submitStep2(
                req.user.id,
                req.body,
            );
            res.json({
    ...
    async submitStep3(req, res, next) {
        try {
            const result = await sellerService.submitStep3(
                req.user.id,
                req.body,
            );
            res.json({
    ...
    async completeRegistrationWithUploads(req, res, next) {
        try {
            ...
            try {
                ...
                throw Object.assign(
                    new Error("Invalid JSON in payload field"),
                    { statusCode: 400 },
                );
            }
            const result = await sellerService.completeRegistrationWithUploads(
                req.user.id,
                req.files || {},
                payload,
            );
    ...
    async verifySensitiveReauth(req, res, next) {
        try {
            ...
            const data = await sellerService.verifySensitiveReauth(
                req.user.id,
                currentPassword,
            );
            res.json({ success: true, data });
    ...
    async getMaskedSensitiveSummary(req, res, next) {
        try {
            const { currentPassword } = req.body;
            const data = await sellerService.getMaskedSensitiveSummary(
                req.user.id,
                currentPassword,
            );
            res.json({ success: true, data });
    ...
    async submitSensitiveChangeRequest(req, res, next) {
        try {
            const result = await sellerService.submitSensitiveChangeRequest(
                req.user.id,
                req.body,
            );
    ...
    async getMyPendingSensitiveChange(req, res, next) {
        try {
            const data = await sellerService.getMyPendingSensitiveChange(
                req.user.id,
            );
            res.json({ success: true, data });
        } catch (error) {
