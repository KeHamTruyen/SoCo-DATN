...
class UserController {
  async getUserByUsername(req, res, next) {
    try {
      ...
      const user = await userService.getUserByUsername(req.params.username, viewerId);
  ...
  async getUserById(req, res, next) {
    try {
      const viewerId = req.user?.id || null;
      const user = await userService.getUserById(req.params.userId, viewerId);
      res.json({ success: true, data: user });
    } catch (error) {
      if (error.message === 'User not found') {
        return res.status(404).json({ success: false, message: error.message });
  ...
  async updateProfile(req, res, next) {
    try {
      const { fullName, username, phone, bio, avatarUrl, coverImage, address, shopInformation } = req.body;
      const user = await userService.updateProfile(req.user.id, {
        fullName, username, phone, bio, avatarUrl, coverImage, address, shopInformation,
      });
