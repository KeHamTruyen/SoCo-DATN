...
class AuthService {
  ...
  async verify2FA(data: { tempToken: string; otpCode: string }): Promise<AuthResponse> {
  ...
  async updatePrivacy(data: {
    ...
    const response = await api.put('/auth/privacy', data);
    return response.data;
  }

  async enable2FA(): Promise<{ success: boolean; data: { message: string; backupCodes: string[] } }> {
    const response = await api.post('/auth/2fa/enable');
    return response.data;
  }

  async confirm2FAEnable(
    otpCode: string
  ): Promise<{ success: boolean; data: { message: string; backupCodes: string[] } }> {
    const response = await api.post('/auth/2fa/confirm', { otpCode });
    return response.data;
  }
