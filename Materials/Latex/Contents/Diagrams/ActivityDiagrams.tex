...
\textbf{UC1.8 – Đăng ký Người bán:} Buyer bắt đầu hồ sơ Seller và hoàn thành tuần tự 3 bước (thông tin cá nhân, thông tin kinh doanh, thông tin ngân hàng). Sau bước 3, hệ thống chuyển hồ sơ sang trạng thái \textit{reviewing} để Admin duyệt. Nếu được duyệt, hệ thống cập nhật vai trò từ \textit{buyer} sang \textit{seller} và gửi email thông báo; nếu bị từ chối, hệ thống lưu lý do để người dùng bổ sung hồ sơ.
...
\textbf{UC2.3 – Gắn thẻ sản phẩm:} Seller chọn một sản phẩm từ shop để gắn vào bài đăng. Người dùng xem bài đăng có thể nhấn vào thẻ sản phẩm để chuyển đến trang chi tiết.
...
\textbf{UC2.6 – Nhắn tin 1-1:} Người dùng A mở hoặc tạo cuộc hội thoại trực tiếp với B, sau đó gửi tin nhắn. Hệ thống lưu tin nhắn vào database, phát sự kiện real-time theo phòng hội thoại qua WebSocket (Socket.io), và tạo thông báo cho người nhận khi đang offline.
...
\textbf{UC2.8 – Báo cáo vi phạm:} Người dùng A chọn đối tượng báo cáo (bài đăng, bình luận, người dùng hoặc sản phẩm), chọn lý do vi phạm và thêm mô tả (nếu có). Hệ thống lưu báo cáo với trạng thái \textit{pending} và hiển thị "Báo cáo đã được ghi nhận" để Admin xử lý trong màn hình quản trị.
...
\textbf{UC3.3 – Thêm vào giỏ hàng:} Buyer nhấn "Thêm vào giỏ hàng". Hệ thống xác định biến thể theo \textit{variantId} (hoặc map từ lựa chọn cũ), kiểm tra trạng thái sản phẩm \textit{ACTIVE} và tồn kho của biến thể/sản phẩm. Nếu hợp lệ, hệ thống upsert cart item và cập nhật số lượng.
...
\textbf{UC3.5 – Thanh toán:} Buyer nhấn "Thanh toán". Hệ thống kiểm tra tồn kho lần cuối theo từng cart item, chốt đơn giá tại thời điểm đặt hàng, tạo Order với trạng thái \textit{PENDING} và \textit{paymentStatus=PENDING}, sau đó trừ tồn kho, làm rỗng giỏ hàng và gửi thông báo "đơn mới" cho Seller.

\textbf{UC3.6 – Theo dõi đơn hàng:} Buyer xem "Đơn hàng của tôi", hệ thống trả danh sách theo bộ lọc/pagination (\textit{/my/purchases}) và chi tiết đơn hàng theo \textit{orderId}. Buyer theo dõi trạng thái qua danh sách và thông báo giao dịch.

\textbf{UC3.7 – Đánh giá sản phẩm:} Khi \textit{order item} thuộc đơn hàng \textit{DELIVERED/COMPLETED}, Buyer gửi đánh giá theo \textit{orderItemId}. Hệ thống đảm bảo mỗi order item chỉ được đánh giá một lần, lưu review và hiển thị trên trang sản phẩm.
...
\textbf{UC3.10 – Quản lý đơn hàng:} Seller lấy danh sách đơn thuộc shop (\textit{/my/sales}), xem chi tiết và cập nhật trạng thái theo luồng hợp lệ \textit{PENDING $\rightarrow$ CONFIRMED $\rightarrow$ PROCESSING $\rightarrow$ SHIPPING $\rightarrow$ DELIVERED}. Buyer chỉ xác nhận hoàn tất (\textit{DELIVERED $\rightarrow$ COMPLETED}).
...
\textbf{UC3.11 – Xử lý hoàn tiền:} Buyer gửi yêu cầu hoàn tiền qua \textit{/refund-request} cho đơn hàng \textit{DELIVERED/COMPLETED}. Seller xem danh sách yêu cầu ở \textit{/seller/refunds}, quyết định chấp nhận/từ chối qua \textit{/refund}. Hệ thống lưu quyết định, cập nhật trạng thái \textit{REFUNDED} khi chấp nhận và gửi thông báo cho Buyer.
